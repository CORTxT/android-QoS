...
public abstract class DeviceInfo {
	...
	public Integer getMNC() {
		String networkOperator = mTelephonyManager.getNetworkOperator();
		if(networkOperator != null && networkOperator.length() > 3) {
			String mnc = DeviceInfo.fixMNC(networkOperator.substring(3));
			int MNC = 0;
			try{
	...
	public static String getIPAddress() {
		try {
			...
			LoggerUtil.logToFile(LoggerUtil.Level.ERROR, "Event", "getLocalIpAddress", "", ex);
	...
	public int getAppVersion() {
		try {
			...
			LoggerUtil.logToFile(LoggerUtil.Level.ERROR, TAG, "getAppVersion", "failed to get app version name", e);
			return 0;
		}
	}
