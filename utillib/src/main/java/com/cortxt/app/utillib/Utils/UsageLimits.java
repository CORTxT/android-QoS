...
//    	}
	...
	public boolean exceededGps (EventType eventType, boolean increment)
	{
        ...
        if (eventType == EventType.EVT_DROP || eventType == EventType.EVT_CALLFAIL || eventType == EventType.MAN_PLOTTING || eventType == EventType.EVT_FILLIN ||
				...
				eventType == EventType.EVT_VQ_CALL || eventType == EventType.SIP_VQ_CALL || eventType == EventType.YOUTUBE_TEST || eventType == EventType.EVT_TEST911 ||
		...
		if (_countGpsFails >= _gpsFailsPer3hr || _countGpsAttempts >= _gpsAttemptsPer3hr)
		{
			if (_countGpsFails >= _gpsFailsPer3hr)
				LoggerUtil.logToFile(LoggerUtil.Level.DEBUG, "UsageLimits", "exceededGps", "countGpsFails > gpsFailsPer3hr");
			else
				LoggerUtil.logToFile(LoggerUtil.Level.DEBUG, "UsageLimits", "exceededGps", "_countGpsAttempts > _gpsAttemptsPer3hr");
			return true;
		}
		else
			return false;
	...
	public int allowSpeedTest (int bytes)
	{
		...
		LoggerUtil.logToFile(LoggerUtil.Level.DEBUG, TAG, "allowSpeedTest", log);
		if (_countSpeedDay >= _speedtestsPerDay || _bytesSpeedMonth >= allowedMB * 1000000 
				|| _bytesSpeedDay >= allowedMB*1000000/12)
		{
			LoggerUtil.logToFile(LoggerUtil.Level.DEBUG, TAG, "allowSpeedTest", "Not Allowed");
			return 0;
		}
		return 1;
	}
