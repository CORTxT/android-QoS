...
public class EventResponse {
    ...
    public void init() {
        if (d.extra_settings != null) {
            try {
                ...
                LoggerUtil.logToFile(LoggerUtil.Level.ERROR, "EventResponseContents", "exception in json ", d.extra_settings, e);
            }
        }
    }
    ...
    public void handleEventResponse(MainService owner, boolean commandsOnly)
    {
        if(this.getCommand() != null && this.getCommand().length() > 2) {
            Intent intent = new Intent(IntentHandler.COMMAND);
            String cmds = this.getCommand().toString ();
            intent.putExtra(IntentHandler.COMMAND_EXTRA, cmds);
            intent.putExtra("STARTTIME_EXTRA", d.starttime);
            owner.sendBroadcast(intent);
        }
        if (this.getSurvey() != null && this.getSurvey() > 0)
        {
            Intent intent = new Intent(IntentHandler.SURVEY);
            intent.putExtra(IntentHandler.SURVEY_EXTRA, this.getSurvey());
            owner.sendBroadcast(intent);
        }
        ...
        {
            Intent intent = new Intent(IntentHandler.SPEED_TEST);
