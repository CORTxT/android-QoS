...
public class GpsManagerOld implements GpsStatus.Listener, LocationListener {
	...
	public GpsManagerOld(MainService owner){
		this.owner = owner;
		try{
		...
		{
			LoggerUtil.logToFile(LoggerUtil.Level.DEBUG, TAG, "GpsManagerOld", "Exception", e);
		}
		catch (Error e)
		{
			LoggerUtil.logToFile(LoggerUtil.Level.ERROR, TAG, "addListenerToCollection", "Error: " + e.toString());
		}
	}
...
//			gpsStatus = locManager.getGpsStatus(null);
	...
	public void coldStart (String reason)
	{
		...
		if (reason.equals("triggered by user"))
		{
			...
			LoggerUtil.logToFile(LoggerUtil.Level.DEBUG, TAG, "coldStart", reason);
...
//		public void run() {
	...
	private void addListenerToCollection(GpsListener listener){
		synchronized(GpsManagerOld.this) {
			...
			if(LoggerUtil.isDebuggable() && listener.getProvider().equals(LocationManager.GPS_PROVIDER)) {
...
//						);
			...
			{
				LoggerUtil.logToFile(LoggerUtil.Level.ERROR, TAG, "addListenerToCollection", "GPS needs to start from main thread: ", e);
			}
			catch (Error e)
			{
				LoggerUtil.logToFile(LoggerUtil.Level.ERROR, TAG, "addListenerToCollection", "Error: " + e.toString());
